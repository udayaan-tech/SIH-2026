...
export default function DashboardPage() {
  ...
  return (
    <div>
      ...
      <div className="page-header-bar">
        <div className="page-title-group">
          ...
          <div className="page-subtitle">
            {isHindi ? 'स्वागत है' : 'Welcome back'}, <strong>{currentOfficer.full_name}</strong> ({currentOfficer.designation})
          </div>
        </div>
        <div className="page-actions">
          <button className="btn btn-secondary btn-sm" onClick={() => router.push('/security-center')}>
            <span style={{ color: 'var(--gov-green)', fontSize: '14px' }}>●</span> System Status: SECURE
          </button>
          <button className="btn btn-primary btn-sm" onClick={() => router.push('/cases')}>
            + Manage Cases
          </button>
        </div>
      </div>
      ...
      <div style={{ backgroundColor: '#FFFFFF', border: '1px solid var(--gov-border)', borderRadius: 'var(--radius-sm)', padding: '10px 16px', marginBottom: '20px', display: 'flex', flexWrap: 'wrap', gap: '20px', fontSize: '12px', color: 'var(--gov-text-secondary)' }}>
        <div><strong>Current Date:</strong> {today}</div>
        <div><strong>Last Login:</strong> {currentOfficer.last_login || 'Today, 13:42:15 IST'}</div>
        <div><strong>Department:</strong> {currentOfficer.department_name || 'Economic Investigation Unit'}</div>
        <div><strong>Security Clearance:</strong> <span className="badge badge-confidential">TIER-1 RESTRICTED</span></div>
        <div><strong>Hardware MFA:</strong> <span style={{ color: 'var(--gov-green)', fontWeight: 700 }}>✓ ACTIVE</span></div>
      </div>
